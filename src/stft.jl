struct Stft{E,W<:WindowedArray,P<:Plan} <: Downsampler{E,1}
    winput::W
    fs::Float64
    win::Vector{Float64}
    ...
    function Stft{E,W,P}(
        ...
        new(winput, fs, win, winfun, r, r_temp, plan, winbuf, frequencies, nfft, nout)
    end
end
...
function Stft(
    ...
    Stft{Vector{Complex{Float64}},W,P}(winput, plan, convert(Float64, fs), winfun)
end
...
function getindex_unsafe!(out, s::StftPsd, i::Integer, d_off::Integer = 1)
    ...
    fft2pow_unsafe!(out, s.fftbuf, s.stft.nfft, s.stft.r_temp[1], d_off, s.stft.nout)
end
