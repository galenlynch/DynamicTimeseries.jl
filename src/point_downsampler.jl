...
function point_extent_averager(
    ::Type{P}
) where {E<:Real, M<:Real, P<:MarkedPoint{E, M}}
    F = div_type(E)
    L = div_type(M)
    R = Tuple{Tuple{E, E}, L}
...
struct DynamicPointBoxer{
    ...
    D<:DynamicPointDownsampler{
        <:Any, <:Any, PtType,<:Any, <:Any
    }
...
function DynamicPointBoxer(
    ...
    DynamicPointBoxer{PtDomainT, PtRangeT, PtType, D}(
        dp, convert(PtDomainT, min_width), convert(PtRangeT, y_center)
    )
end
