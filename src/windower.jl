...
function WindowedArray(
    ...
    WindowedArray{typeof(v),T,N,A}(input, convert(Int, binsize), convert(Int, overlap))
end

function validate_windower_args(
    input_dims::NTuple{N,Integer},
    binsize::Integer,
    overlap::Integer,
) where {N}
        ...
        throw(
            ArgumentError(
                string(
                    "binsize is ",
                    binsize,
                    " but it must be between 1 and the input length, ",
                    input_len,
                ),
            ),
        )
        ...
        throw(
            ArgumentError(
                string(
                    "overlap is ",
                    overlap,
                    " but it must be between 0 and binsize, ",
                    binsize,
                ),
            ),
        )
