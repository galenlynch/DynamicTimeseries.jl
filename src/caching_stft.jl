...
function CachingStftPsd(sp::StftPsd, base_offset::Real = 0, args...; kwargs...)
    new_fs = sp.stft.fs / sp.stft.winput.binsize
    new_offset = base_offset + (sp.stft.winput.binsize / sp.stft.fs)
    CachingStftPsd(CacheAccessor(Averager, sp, new_fs, new_offset, args...; kwargs...))
end
...
function CachingStftPsd(
    ...
    CachingStftPsd(StftPsd(input, binsize, fs, overlap, winfun), args...; kwargs...)
end
