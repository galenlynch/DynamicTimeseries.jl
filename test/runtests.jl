using DynamicTimeseries, EventIntervals
using Test, Random, Statistics
Random.seed!(1)

@testset "DynamicTimeseries" begin
    ...
    @testset "timeseries" begin
        @testset "WindowedArray" begin
            ...
            wa[end]
            wa_overlap = WindowedArray(A, 6, 2)
        ...
        @testset "maxmin" begin
            ...
            @test length(mm) == 100
            @test size(mm) == (100,)
            @test DynamicTimeseries.binsize(mm) == 10
            @test mm[1] == (1, 2)
            @test mm[end] == (1, 2)
            mm[1:-1]
            mm2 = MaxMin(B, 10)
        ...
        @testset "AveragingTs" begin
            davg = DynamicDownsampler(Averager, B, fs)
            (xs, ys, wd) = downsamp_req(davg, 0, 10, 2)
        end

        @testset "DynamicTs" begin
            dts = DynamicDownsampler(MaxMin, A, 10)
        ...
        @testset "CachingDynamicTs" begin
            ...
            @testset "Caching" begin
                ...
                DynamicTimeseries.sort_cache_files(files, dims)
                cachedir = tempdir()
                (files, E, lengths) =
            ...
            @testset "CacheAccessor" begin
                ...
                @testset "MaxMin" begin
                    ...
                    @test DynamicTimeseries.extrema_red(ys) == true_extrema
                end
